Title: Agrégation
Date: 2026-04-19
Category: Agrégation

The agrégation de mathématiques is a competitive French national exam for recruiting advanced mathematics teachers. This section is written in French because the exam itself is in French.

## Ressources

+ Méthodologie pour l'écriture d'une leçon _(coming soon)_
+ Mes avis sur les différents livres que j'ai utilisé _(coming soon)_
+ Tableau de couplage/organisation vierge: [analyse](/files/Organisation_Oral_Analyse.xlsx) / [algèbre](/files/Organisation_Oral_Algebre.xlsx)

## Conseils

+ Préparer l'écrit et l'oral en même temps. Privilégier des développements qui correspondent à des démonstrations utiles pour l'écrit.
+ Ne pas choisir de développements trop exotiques. Ils sont plus difficiles à retenir, souvent moins utiles et les questions posées par le jury seront plus exigeantes. 
+ Lire les préfaces des livres utilisés pour les leçons. Cela permet parfois de comprendre comment il a été pensé et ainsi l'utiliser à bon escient.

## Site Web utiles

+ Les basiques: [Site Officiel](https://agreg.org/), [Agreg-Maths](https://agreg-maths.fr/).
+ Les copains: [TC&WM](https://sites.google.com/view/tribalchiefandwiseman/), [Clara Genes](https://perso.eleves.ens-rennes.fr/people/clara.genes/index.html), [Alexis Guerin](https://aguerin.pages.math.cnrs.fr/).